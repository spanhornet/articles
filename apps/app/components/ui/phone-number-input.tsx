...
const FlagComponent: React.FC<RPNInput.FlagProps> = ({ country, countryName }) => {
  ...
  return (
    <span className="w-5 overflow-hidden rounded-sm hidden">
      {Flag ? <Flag title={countryName} /> : <GlobeIcon aria-hidden="true" />}
    </span>
  )
}
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button
          ref={buttonRef}
          disabled={disabled}
          className="h-9 border-input bg-background text-foreground hover:bg-accent inline-flex items-center justify-center gap-2 self-stretch rounded-l-md border py-2 px-2 transition-all outline-none disabled:pointer-events-none disabled:opacity-50"
          type="button"
        >
          <span className="w-5 overflow-hidden rounded-xs ml-1">{Flag ? <Flag title={selectedCountry?.label || ""} /> : <GlobeIcon aria-hidden="true" />}</span>
          <CaretSortIcon className="text-muted-foreground" aria-hidden="true" />
        </button>
      </PopoverTrigger>
                ...
                >
                  <div className="flex items-center gap-2">
                    <span className="w-5 overflow-hidden rounded-sm">
                      {flags[country.value] ? React.createElement(flags[country.value]!, { title: country.label }) : <GlobeIcon aria-hidden="true" />}
                    </span>
                    <span>{country.label}</span>
                    <span className="text-muted-foreground">{`+${country.code}`}</span>
                    {country.value === value && <CheckIcon className="ml-auto" aria-hidden="true"/>}
                  </div>
                </CommandItem>
              ))}
            </CommandGroup>
